...
def obrnjen(vs):
    tab = []
    voz = vs._zacetek
    while voz is not None:
        tab.append(voz.podatek)
        voz = voz.naslednji
    tab = tab[::-1]
    v = VerizniSeznam()
    for i in tab:
...
def obrni(vs):
    tab = []
    while vs._zacetek.naslednji is not None:
        tab.append(vs._zacetek.podatek)
        vs._zacetek = vs._zacetek.naslednji
    tab = tab[::-1]
    for i in tab:
        vs.vstavi_na_konec(i)
    return vs
# =====================================================================@032138=
# 2. podnaloga
...
def filtriraj(vs, f):
    nov = VerizniSeznam()
    pomozen = VerizniSeznam()
    while not vs.je_prazen():
        el = vs.zacetek()
        if f(el):
            pomozen.vstavi_na_zacetek(el)
        else:
            nov.vstavi_na_zacetek(el)
        vs.izbrisi_zacetek()
    while not pomozen.je_prazen():
        el = pomozen.zacetek()
        vs.vstavi_na_zacetek(el)
        pomozen.izbrisi_zacetek()
    return nov
...
from verizni_seznami_z_dolzino import VerizniSeznam

def razdrobi_glede_na_enke(vs):
    t = [VerizniSeznam() for _ in range(vs.dolzina())]
    while not vs.je_prazen():
        x = vs.zacetek()
        mesto = 0
        for i in str(x):
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzNywidXNlciI6MjcwMn0:1oxll7:LzoOmndGKKf8xMM4N-jqVwIj59BCHbtTvTdNAXHLLWA'
        try:
            tests = [
                (['s = VerizniSeznam()',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzOCwidXNlciI6MjcwMn0:1oxll7:TuQIBfkufIqK5rdQWWPu9KOxygaA-bQsSamgEO6SeUs'
        try:
            tests = [
                (['s = VerizniSeznam()',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzOSwidXNlciI6MjcwMn0:1oxll7:dINkoEeLt6b9RxseDFKz7xAapoKtobb0qnCZ73lGlgQ'
        try:
            tests = [
                (['vs = VerizniSeznam()',
                  'vs.vstavi_na_zacetek(0)',
