...
def stevilo_dni(leto):
    return (365 if not je_prestopno(leto) else 366)
...
class Datum(Datum):
    def je_veljaven(self):
        '''ali d. m. l predstavlja
           veljaven datum '''
        if self.leto < 0: return False
        return (1 <= self.mesec <= 12 and
                1 <= self.dan <= dolzine_mesecev(self.leto)[self.mesec - 1])
...
class Datum(Datum):
    ...
    def __eq__(self, other):
        if self.leto == other.leto and self.mesec == other.mesec and self.dan == other.dan:
            return True
        else:
            return False
...
class Datum(Datum):
    ...
    def dni_od_zacetka(self):
        st_dni = 0
        for i in range(self.leto):
            st_dni += stevilo_dni(i)
        st_dni += self.dan_v_letu()
        return st_dni
...
class Datum(Datum):
    ...
    #     k = self.leto % 100
    #     j = self.leto // 100
    #     h = (self.dan + math.floor(13*(self.mesec + 1)/5) + k + math.floor(k/4) + math.floor(j/4) - (2 * j)) % 7
    #     return h

    def dan_v_tednu(self):
        pon = Datum(10, 10, 2022).dni_od_zacetka()
        rez = pon - self.dni_od_zacetka()
        dni_nazaj = (rez) % 7
        if dni_nazaj == 1:
            return 7
        if dni_nazaj == 2:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk0NCwidXNlciI6MjcwMn0:1oiX4f:7XZ3dQgv2kNlo0HYjoZCcVk1Gtk'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk0NSwidXNlciI6MjcwMn0:1oiX4f:mHAaC7yF1hm_eFg4Pvs12xvG1q8'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk0NiwidXNlciI6MjcwMn0:1oiX4f:VQZnNS0O1HNfKO-FFOwccSfsdQg'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk0NywidXNlciI6MjcwMn0:1oiX4f:XPHplk8rebd43QMRI_IZ3IsKkQ0'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk0OCwidXNlciI6MjcwMn0:1oiX4f:mYSAtUN_BDGDQnx-RWIj_q88WDQ'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk0OSwidXNlciI6MjcwMn0:1oiX4f:7uwyvNK9u66nbNlo4woPD_6hp10'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1MCwidXNlciI6MjcwMn0:1oiX4f:fm0UW4_-2QhO1SbQ5VVTwhH22tU'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1MSwidXNlciI6MjcwMn0:1oiX4f:V_dtVsqDSgvL63tTgukJupkZA70'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1MiwidXNlciI6MjcwMn0:1oiX4f:rBD0GoU4JhNuKn1SmVolc6I3Lqk'
        try:
            tests = [
                    ('Datum(31, 12, 1999) == Datum(1, 1, 2000)', False),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1MywidXNlciI6MjcwMn0:1oiX4f:GFRznSMZZIHBTddbyj09HXLlsFA'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1NCwidXNlciI6MjcwMn0:1oiX4f:y4VWWLrP7bQkHasHc4QSG5Gu0Hk'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1NSwidXNlciI6MjcwMn0:1oiX4f:85szJwFKyDwzDcKIm4NTW-aWQeo'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMTk1NiwidXNlciI6MjcwMn0:1oiX4f:BxX-c_Om5ZmC_46lb9oju23soyQ'
        try:
            tests = [
