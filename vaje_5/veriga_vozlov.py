...
def vrni_seznam_test(prvi):
    ...
    if prvi == Vozel(3, Vozel(2)):
        return [3, 2]
    if prvi == Vozel('petek', Vozel('sobota', Vozel('nedelja'))):
        return ['petek', 'sobota', 'nedelja']
    if prvi == Vozel(10, Vozel(5, Vozel('a', Vozel(None, Vozel(1))))):
        return [10, 5, 'a', None, 1]
...
def vrni_seznam(prvi):
    ...
    if prvi is None:
        return tab
    elif prvi.naslednji is None:
        tab.append(prvi.podatek)
    else:
        tab.append(prvi.podatek)
        while not prvi.naslednji is None:
            prvi = prvi.naslednji
            tab.append(prvi.podatek)
    return tab
...
class Check:
    ...
    @staticmethod
    def generator(expression, expected_values, should_stop=None, further_iter=None, clean=None, env=None,
                  update_env=None):
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE3OH0:1miIch:_V4gzSbAY-wi7c7saynC-1UxY7M'
        try:
            Check.equal("vrni_seznam(Vozel(3, Vozel(2)))", [3, 2])
            Check.equal("vrni_seznam(Vozel('petek', Vozel('sobota', Vozel('nedelja'))))",
                        ['petek', 'sobota', 'nedelja'])
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE3OX0:1miIch:aprkWj3H3pxJXwS1xB4tDUM5wjM'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE4MH0:1miIch:HnW0EGVrB3oiMrv9btQ1tbVU5zc'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE4MX0:1miIch:t7Nzgo-vuI4n0Qg5tazB4gHXL1s'
        try:
            test_data = [
                (["v = iz_seznama([])",
                  "l = vrni_seznam(v)"],
