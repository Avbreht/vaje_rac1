...
class DruzabnaVrsta:
    ...
    def dodaj_prisleka(self, ime, skupina):
        ...
        while not self.vrsta.prazna():
            vrh = self.vrsta.zacetek()
            pomozna.vstavi((vrh[0], vrh[1]))
            if vrh[1] == skupina:
                pomozna.vstavi((ime, skupina))
                obstoj += 1
            self.vrsta.odstrani()
        if obstoj == 0:
            pomozna.vstavi((ime, skupina))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjA5NSwidXNlciI6MjcwMn0:1oqBn2:CCQt3K_Jku0w1oMfdbPb0LMAgRA'
        try:
