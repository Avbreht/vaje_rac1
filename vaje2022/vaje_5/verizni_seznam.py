...
# =============================================================================
    ...
    def izbrisi_konec(self):
        ...
        if self._zacetek == self._konec:
            ...
            el = self._zacetek
            while el.naslednji != self._konec:
                el = el.naslednji
            self._konec = el
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEyNywidXNlciI6MjcwMn0:1oxlky:C4Xpu0sgOJB1EoAU3uUExTXAnurYhD0tfsYqif77qho'
        try:
            Check.run([
                's = VerizniSeznam()',
                'niz = str(s)',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEyOCwidXNlciI6MjcwMn0:1oxlky:9iXF7MYxSkIMq_wuGQUIhaQ0L0GPwZXB3mepM28SQuw'
        try:
            Check.run([
                's = VerizniSeznam()',
                's.vstavi_na_zacetek(10)',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEyOSwidXNlciI6MjcwMn0:1oxlky:2cBhgLTPUYkAQQzv3tEhYSH5gcclOaS8kurTCpUOxUY'
        try:
            Check.run([
                's = VerizniSeznam()',
                's.vstavi_na_zacetek(10)',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzMCwidXNlciI6MjcwMn0:1oxlky:mVSbWqZl_yTXbKbQrd8812KoBT67lphdsO9OtmtyGD4'
        try:
            Check.run([
                's = VerizniSeznam()',
                'niz = str(s)',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzMSwidXNlciI6MjcwMn0:1oxlky:2zYXnJw-A7auiYk-vuTDTnZ6Gd_9ndITulnUUST6wq4'
        try:
            Check.run([
                's = VerizniSeznam()',
                's.vstavi_na_zacetek(10)',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzMiwidXNlciI6MjcwMn0:1oxlky:aXo7I_qXnOsvd17I9NjAT7o3qr5JvFQlKMuG3UnZJGU'
        try:
            Check.run([
                's = VerizniSeznam()',
                's.vstavi_na_konec(10)',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzMywidXNlciI6MjcwMn0:1oxlky:PygZqDVzU0Taok0uNtLapT0aDlS6EOt0mp3zMOJbAvw'
        try:
            Check.run([
                's = VerizniSeznam()',
