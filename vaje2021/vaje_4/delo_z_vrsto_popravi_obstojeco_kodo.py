...
#     pomozni = Sklad()
#     stevec = 0
#     while not v.prazna() or stevec > 0:
#         pomozni.vstavi(v.zacetek())
#         v.odstrani()
#         stevec += 1
#         if v.zacetek == pomozni.vrh():
#             stevec = 0
#             while pomozni.prazen():
#                 pomozni.odstrani()
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE2MH0:1mg2yc:yv29EsEHfx08L040LhyJleDa7pk'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE2MX0:1mg2yc:2cgY19nOPwsdT4BWJlHtti9MIGA'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE2Mn0:1mg2yc:Oc0K5jQPukkvzGN2noujncvZ7S8'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "odstrani_ntega(v, 3)",
                       "potem1 = str(v)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE2M30:1mg2yc:NTboEvCtDXvoV5WYAN_a6jmPnjc'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "odstrani_pogojno(v, lambda x: x % 2 == 0)",
                       "potem = str(v)"],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE2NH0:1mg2yc:6yyz9So_4AmJTuvnECg30qQlRGA'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "obrni(v)",
                       "potem = str(v)"],
