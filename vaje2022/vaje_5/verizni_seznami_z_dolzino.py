...
#
# Verižni seznam lahko nadgradimo tako, da hkrati hrani tudi svojo dolžino. Če
# pogosto potrebujemo dolžino seznama, hkrati pa uporabljamo zelo obsežne
# sezname, je hranjenje dolžine učinkovitejše kot računanje.
# =====================================================================@032134=
# 1. podnaloga
# Dopolnite definicijo verižnega seznama z metodami (kot so opisane v nalogi
# *Verižni seznam*):
# 
# * `vstavi_na_zacetek`
# * `zacetek`
...
# 
# **Namig:** prekopirajte vašo rešitev naloge *Verižni seznam* in metode
# primerno popravite.
# =============================================================================
class Vozel:
    '''
...
class VerizniSeznam:
    ...
    def vstavi_na_zacetek(self, podatek):
        '''Na začetek seznama vstavi podatek.'''
        if self._zacetek is not None:
            self._zacetek = Vozel(podatek, self._zacetek)
        else:
            self._zacetek = self._konec = Vozel(podatek)
        self._dolzina = self._dolzina + 1

    def zacetek(self):
        '''Vrni podatek na začetku seznama.'''
        if self._zacetek:
    ...
    def vstavi_na_konec(self, podatek):
        '''Na konec seznama vstavi podatek.'''
        if self._konec:
    ...
    def izbrisi_konec(self):
        ...
        if self._zacetek and self._zacetek.naslednji:
            ...
            while vozel.naslednji.naslednji is not None:
                vozel = vozel.naslednji
            assert vozel.naslednji == self._konec
            self._konec = vozel
    ...
    def je_prazen(self):
        '''Vrne True, kadar je seznam prazen, in False, kadar ni.'''
...
# leksikografsko. Pazite, da metoda ne spremeni začetnih seznamov.
# =============================================================================

    def __lt__(self, other):
        if self.dolzina() < other.dolzina():
            return True
        elif self.dolzina() > other.dolzina():
            return False
        else:
            d = self.dolzina()
            je_najmansi = None
            for _ in range(d):
                el1 = self.zacetek()
                el2 = other.zacetek()
                if je_najmansi is None:
                    if el1 < el2:
                        je_najmansi = True
                    elif el1 > el2:
                        je_najmansi = False
                self.izbrisi_zacetek()
                self.vstavi_na_konec(el1)
                other.izbrisi_zacetek()
                other.vstavi_na_konec(el2)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzNCwidXNlciI6MjcwMn0:1oxll5:FzZQpqfxFzLo85N7U7eAE-hUi4S3v9n0x4-A_UYEyRA'
        try:
            tests = [
                (['s = VerizniSeznam()',
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzNSwidXNlciI6MjcwMn0:1oxll5:3y7qQa1E_lmOwGWYHy_OjspAHxT5hm7nqLF9zg1K2JQ'
        try:
            tests = [
                (['s = VerizniSeznam()',
                  's.vstavi_na_zacetek(10)',
                  's.vstavi_na_konec(20)',
                  's.vstavi_na_konec(30)',
                  's.filtriraj(lambda x: x > 12)',
                  'niz = str(s)',
                  'd = s.dolzina()'],
                 {'niz': '20 -> 30 -> •', 'd': 2}),
                (['s = VerizniSeznam()',
                  's.filtriraj(lambda x: False)',
                  'niz = str(s)'],
                 {'niz': '•'}),
            ]
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMjEzNiwidXNlciI6MjcwMn0:1oxll5:8jMlYnkHpzPFrHbGyrFZPZVVSrmvWICVq0C7qhpbWCI'
        try:
            tests = [
                (['s1 = VerizniSeznam()',
                  's2 = VerizniSeznam()',
                  's3 = VerizniSeznam()',
                  's4 = VerizniSeznam()',
                  's2.vstavi_na_konec(10)',
                  's3.vstavi_na_konec(10)',
                  's4.vstavi_na_konec(10)',
                  's2.vstavi_na_konec(20)',
                  's3.vstavi_na_konec(22)',
                  's2.vstavi_na_konec(40)',
                  's3.vstavi_na_konec(30)',
                  'niz = [str(s) for s in sorted([s1, s2, s3, s4])]'],
                 {'niz': ['•', '10 -> •', '10 -> 20 -> 40 -> •', '10 -> 22 -> 30 -> •']}),
            ]
