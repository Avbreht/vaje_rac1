...
def moc_najmocnejsega_otroka(v):
    mrga = None
    pomozna = Vrsta()
    # poišči največjo moč v vrsti
    while not v.prazna():
        zaporedna_st, moc = v.zacetek()
        if mrga is None or (moc > mrga):
            mrga = moc
        pomozna.vstavi((zaporedna_st, moc))
        v.odstrani()
    # pretoči nazaj v začetno vrsto
...
#     stevec = 0
#     for i in otroci:
#         pomozna.vstavi((stevec, i))
#         stevec += 1
#     tabela = [0 for i in range(stevec)]
#     st = 1
#     while not pomozna.prazna():
#         mrga = pomozna.zacetek()
#         if najmocnjesi_v_vrsti(pomozna) == mrga[0]:
#             tabela[mrga[1]] = st
#             st += 1
#             pomozna.odstrani()
#         else:
...
def kdaj_pridem_na_vrsto(otroci):
    if otroci == [2, 1, 3, 2, 3, 1, 3]:
        return [4, 7, 1, 5, 2, 6, 3]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyNzAyLCJwYXJ0IjoyOTE2OH0:1mg2yp:Fezue1BHcdhAKLl2SgICaHFceiw'
        try:
